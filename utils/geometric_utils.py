...
def line_intersection(line1, line2):
    """Intersection between two line segments defined as (x1,y1,x2,y2)."""
    x1,y1,x2,y2 = line1
    x3,y3,x4,y4 = line2
...
def compute_trapezoid(far_line, near_line, line_left, line_right, height):
    """
    Compute the 4 vertices of the trapezoid from the intersection of the lines.
    Returns np.ndarray shape (4,2) float32 or None.
    """
    if line_left is None or line_right is None:
        return None

    def intersect_with_y(line, y):
        """Returns x where the line intersects the horizontal line y=y."""
        x = extrapolate_line_point(line, y)
        return x

    # Bottom border: use near_line if available, otherwise y=0
    if near_line is not None:
        # Intersect left_line with near_line
    ...
    # Top border: use far_line if available, otherwise y=0
    if far_line is not None:
        # Intersect left_line with far_line
...
def extend_trapezoid(vertices, offset):
    # vertices is np.array([[tl_x, tl_y], [tr_x, tr_y], [br_x, br_y], [bl_x, bl_y]])
    ...
    # Left Side (between TL [0] and BL [3])
    new_vertices[0], new_vertices[3] = extend_pair(vertices[0], vertices[3])

    # Right Side (between TR [1] and BR [2])
